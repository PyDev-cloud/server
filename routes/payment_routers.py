...
from models.payment_model import InvoiceStatus,PaymentInvoice,PaymentDeduction
...
async def create_invoice(data: PaymentInvoiceCreate, db: Session = Depends(get_db)):
    return create_payment_invoice(db, data)

@router.post("/{invoice_id}/approve-finance", response_model=PaymentInvoiceOut)
async def approve_finance(invoice_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def approve_president(invoice_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return approve_payment_president(db, invoice_id, current_user)

@router.get("/pending", response_model=list[PaymentInvoiceOut])
async def list_pending_invoices(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
